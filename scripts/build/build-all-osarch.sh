...
if [ -z "${VERSION}" ]; then
    echo "Error: VERSION is missing. e.g. ./build-all-osarch.sh <build_dir> <name> <version> <build_os_list> <build_arch_list> <build_ldflag>"
    exit 1
fi
if [ -z "${GOLDFLAGS}" ]; then
    echo "Error: GOLDFLAGS is missing. e.g. ./build-all-osarch.sh <build_dir> <name> <version> <build_os_list> <build_arch_list> <build_ldflag>"
    exit 1
fi
